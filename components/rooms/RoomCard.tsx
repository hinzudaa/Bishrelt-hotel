    ...
    <div className="bg-white group cursor-pointer">
      <div className="relative h-[330px] overflow-hidden">
        ...
        <div className="absolute bottom-0 left-0 px-3 py-1.5">
          <span className="text-white font-roboto-slab text-[32px] font-normal">{room.price}</span>
        </div>
        <div className="absolute top-5 right-5 bg-[#0A0A0A66] border-2 border-black px-3 py-1.5">
          <span className="text-white/75 text-[9px] tracking-[0.22em] uppercase">{room.type}</span>
        </div>
      </div>

      <div className="p-8">
        <h3 className="text-black font-cormorant font-semibold text-[28px]">{room.name}</h3>
        <p className="text-gold text-[16px] font-cormorant italic font-normal tracking-wide">{room.subtitle}</p>
